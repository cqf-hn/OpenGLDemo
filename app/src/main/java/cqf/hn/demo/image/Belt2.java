...
public class Belt2 {
    ...
    public Belt2(GLSurfaceView view) {
        initVertexData();
        initShader(view);
    }
    ...
    private void initVertexData() {
        int n1 = 3;//被分割的成的四边形数量
        int n2 = 10;//被分割的成的四边形数量
        vCount = 2 * (n1 + 1) + 1;
        vCount += 2 * (n2 + 1) + 1;
        //确定顶点位置
        float beginAngle = 0;
        float endAngle = 60.0f;
        float angleSpan = (endAngle - beginAngle) / n1;
        float beginAngle2 = 180.0f;
        float endAngle2 = 240.0f;
        float angleSpan2 = (endAngle2 - beginAngle2) / n2;

        float[] vertices = new float[vCount * 3];
        int count = 0;
        for (float angle = beginAngle; angle <= endAngle; angle += angleSpan) {
            ...
            vertices[count++] = 0;
            if (angle + angleSpan > endAngle - beginAngle) {
                vertices[count++] = (float) (Constant.UNIT_SIZE * Math.cos(radians));
                vertices[count++] = (float) (Constant.UNIT_SIZE * Math.sin(radians));
                vertices[count++] = 0;
            }
        }
        for (float angle = beginAngle2; angle <= endAngle2; angle += angleSpan2) {
            double radians = Math.toRadians(angle);
            if (angle == beginAngle2) {
                vertices[count++] = (float) (0.6f * Constant.UNIT_SIZE * Math.cos(radians));
                vertices[count++] = (float) (0.6f * Constant.UNIT_SIZE * Math.sin(radians));
                vertices[count++] = 0;
            }
